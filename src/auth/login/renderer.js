...
const { login, getPrimaryRole, storeAuth, apiRequest } = require('../../../shared/api');
...
ipcRenderer.on('google-auth-success', async (event, tokens) => {
    console.log('Google auth successful, storing and finishing...');
    
    try {
        // Fetch user profile to get roles and full details
        const userData = await apiRequest('/auth/profile', {
            headers: { 'Authorization': `Bearer ${tokens.accessToken}` }
        });
        
        storeAuth(tokens.accessToken, tokens.refreshToken, userData);
        
        const role = getPrimaryRole(userData.roles);
        ipcRenderer.send('login-success', { role });
    } catch (err) {
        console.error('Error fetching profile after Google login:', err);
        showError('Authenticated with Google but failed to fetch profile.');
    }
});
