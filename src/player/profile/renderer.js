...
function renderProfile(user, profile, ranks, friendshipStatus) {
    ...
    if (!isOwnProfile) {
        // Message button (available even if not friends)
        const messageBtn = `<button onclick="window.openChat('${targetUserId}')" class="px-5 py-2 text-xs font-bold bg-[#22d3ee]/20 text-[#22d3ee] rounded-xl border border-[#22d3ee]/30 hover:bg-[#22d3ee]/30 transition-all flex items-center gap-2">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" /></svg>
            MESSAGE
        </button>`;

        if (friendStatus === 'ACCEPTED') {
            friendActionHtml = `<div class="flex items-center gap-3">
                <span class="px-4 py-2 text-xs font-bold text-[#00ff87] bg-[#00ff87]/10 rounded-xl border border-[#00ff87]/20 uppercase tracking-wider">FRIENDS</span>
                ${messageBtn}
            </div>`;
        } else if (friendStatus === 'PENDING') {
            friendActionHtml = `<div class="flex items-center gap-3">
                <span class="px-4 py-2 text-xs font-bold text-yellow-400 bg-yellow-400/10 rounded-xl border border-yellow-400/20 uppercase tracking-wider">REQUEST PENDING</span>
                ${messageBtn}
            </div>`;
        } else {
            friendActionHtml = `<div class="flex items-center gap-3">
                <button id="add-friend-btn" class="px-5 py-2 text-xs font-bold bg-[#00ff87]/20 text-[#00ff87] rounded-xl border border-[#00ff87]/30 hover:bg-[#00ff87]/30 transition-all">ADD FRIEND</button>
                ${messageBtn}
            </div>`;
        }
    }

    // Expose openChat to window for the button
    window.openChat = (uid) => {
        localStorage.setItem('active_chat_user_id', uid);
        window.location.href = '../chat/chat_detail.html';
    };
