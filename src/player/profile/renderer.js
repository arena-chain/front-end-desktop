...
function renderProfile(user, profile, ranks, friendshipStatus) {
    ...
    const steamVerified = user.steamVerified === true;
    ...
    const steamHtml = steamVerified ? `
        <div class="flex items-center gap-2 mt-2">
            <span class="px-2 py-0.5 bg-[#66c0f4]/20 text-[#66c0f4] text-[10px] font-bold rounded border border-[#66c0f4]/30">STEAM VERIFIED</span>
            <span class="text-xs text-gray-400">${esc(user.steamUsername)}</span>
            ${isOwnProfile ? `<button id="steam-unlink-btn" class="ml-2 text-[9px] text-gray-500 hover:text-red-400 transition-colors uppercase font-bold">Unlink</button>` : ''}
        </div>
    ` : '';
    ...
    const unlinkBtn = document.getElementById('steam-unlink-btn');
    if (unlinkBtn) {
        unlinkBtn.addEventListener('click', async () => {
            if (!confirm('Are you sure you want to unlink your Steam account? You will be unable to join matchmaking until you verify it again.')) return;
            try {
                await apiRequest('/steam/unlink', { method: 'DELETE' });
                loadProfile(); // Refresh
            } catch (err) {
