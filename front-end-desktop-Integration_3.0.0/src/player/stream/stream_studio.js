...
const els = {
    ...
    categoryTagsRow: document.getElementById('category-tags-row'),
};
...
async function ensureStream(channel, forceStart = false) {
    ...
    // Add optional scheduling if provided in the session
    if (session.schedule) {
        const start = new Date(session.schedule);
        if (!isNaN(start.getTime())) {
            payload.scheduledStartTime = start.toISOString();
            // User requested NO end date
        }
    }
    ...
    if (!stream.isLive && (forceStart || !session.schedule)) {
        // Only auto-start if it's NOT a future scheduled stream, UNLESS forced (e.g. Go Live click)
...
async function startLive() {
    ...
    try {
        ...
        const stream = await ensureStream(channel, true);
...
async function initStudio() {
    ...
    try {
        const channel = await ensureChannel();
        // If there's a schedule, we create/update the stream draft now so followers get notified
        if (session.schedule) {
            const stream = await ensureStream(channel);
            appendActivity(`Scheduled stream "${stream.title}" is ready.`, 'live');
        ...
        appendActivity('Failed to initialize studio data: ' + error.message, 'danger');
    }
}

function checkSchedule() {
    if (!session.schedule || state.isLive) return;
    ...
    if (currentTime < scheduledTime) {
        ...
        els.goLive.style.opacity = '0.5';
        els.goLive.style.cursor = 'not-allowed';
        ...
        if (hours > 0) {
            els.goLive.textContent = `Starts in ${hours}h ${mins}m`;
        } else {
            els.goLive.textContent = `Starts in ${mins}m`;
        }
    } else {
        els.goLive.disabled = false;
        els.goLive.style.opacity = '1';
        els.goLive.style.cursor = 'pointer';
        els.goLive.textContent = 'Go Live';

        if (!state.notificationShown) {
            state.notificationShown = true;

            // Show a native browser alert for maximum visibility as requested
            window.alert(`READY TO GO LIVE!\n\nYour scheduled stream "${session.title || 'Live'}" is ready to start.`);

            if ('Notification' in window && Notification.permission === 'granted') {
                new Notification('ARENA - Stream Reminder', {
                    body: `It's time! Your scheduled stream "${session.title || 'Live'}" is ready to start.`,
                    icon: '../../assets/logo.png'
                });
            }
            appendActivity('Scheduled time reached! You can now go live.', 'live');
        }
    }
}
...
function renderCategoryTags() {
    const tagStr = draft.category || session.category || '';
    const tags = tagStr.split(',').map(t => t.trim()).filter(Boolean);
    if (!els.categoryTagsRow || tags.length === 0) return;
    els.categoryTagsRow.innerHTML = tags.map(tag =>
        `<button class="category-tag" data-tag="${tag}">${tag}</button>`
    ).join('');
    els.categoryTagsRow.querySelectorAll('.category-tag').forEach(btn => {
        btn.addEventListener('click', () => {
            const cat = btn.dataset.tag;
            const url = `../stream/stream_dashboard.html?category=${encodeURIComponent(cat)}`;
            try { ipcRenderer.send('navigate-to', 'live-streams'); } catch { }
...
if (els.btnMic) {
    els.btnMic.addEventListener('click', () => {
        ...
        if (state.localStream) {
            state.localStream.getAudioTracks().forEach(t => { t.enabled = micEnabled; });
        }
        els.btnMic.className = `ctrl-btn ${micEnabled ? 'on' : 'off'}`;
        els.btnMic.innerHTML = `
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
                ${micEnabled
                ? '<rect x="9" y="2" width="6" height="11" rx="3"/><path d="M5 10a7 7 0 0014 0"/><line x1="12" y1="17" x2="12" y2="21"/><line x1="8" y1="21" x2="16" y2="21"/>'
                : '<rect x="9" y="2" width="6" height="11" rx="3"/><path d="M5 10a7 7 0 0014 0"/><line x1="12" y1="17" x2="12" y2="21"/><line x1="8" y1="21" x2="16" y2="21"/><line x1="1" y1="1" x2="23" y2="23"/>'
            }
            </svg>
            Mic ${micEnabled ? 'ON' : 'OFF'}
        `;
        appendActivity(`Mic ${micEnabled ? 'unmuted' : 'muted'}.`, micEnabled ? 'live' : 'danger');
    });
}
...
if (els.btnCamera) {
    els.btnCamera.addEventListener('click', () => {
        ...
        if (state.localStream) {
            state.localStream.getVideoTracks().forEach(t => { t.enabled = cameraEnabled; });
        }
        els.btnCamera.className = `ctrl-btn ${cameraEnabled ? 'on' : 'off'}`;
        els.btnCamera.innerHTML = `
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
                ${cameraEnabled
                ? '<path d="M23 7l-7 5 7 5V7z"/><rect x="1" y="5" width="15" height="14" rx="2"/>'
                : '<path d="M23 7l-7 5 7 5V7z"/><rect x="1" y="5" width="15" height="14" rx="2"/><line x1="1" y1="1" x2="23" y2="23"/>'
            }
            </svg>
            Camera ${cameraEnabled ? 'ON' : 'OFF'}
        `;
        appendActivity(`Camera ${cameraEnabled ? 'on' : 'off'}.`, cameraEnabled ? 'live' : 'danger');
    });
}
...
if (els.btnFullscreen) {
    els.btnFullscreen.addEventListener('click', () => {
        const target = els.previewVideo.classList.contains('hidden') ? els.previewImage : els.previewVideo;
