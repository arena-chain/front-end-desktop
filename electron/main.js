...
<<<<<<< HEAD
    ...
    setTimeout(() => {
        console.log('[BOT] Spawning python script...');
        const scriptPath = path.join(__dirname, 'antigravity_voice_trigger.py');
        console.log('[BOT] Script path:', scriptPath);
        console.log('[BOT] Script exists:', fs.existsSync(scriptPath));

        const bot = spawn('C:\\Users\\314ckle\\AppData\\Local\\Python\\pythoncore-3.14-64\\python.exe', [
            scriptPath
        ...
        bot.stdout.on('data', (data) => {
            ...
            for (const line of lines) {
                ...
                if (msg.startsWith('NAVIGATE:')) {
                    const route = msg.replace('NAVIGATE:', '').trim();
                    console.log(`[BOT] Navigating to: ${route}`);
                    navigateTo(route);
                }
                if (win) win.webContents.send('bot-status', msg);
            }
        });

        bot.stderr.on('data', (data) => console.error(`[BOT ERR] ${data.toString()}`));
        bot.on('error', (err) => console.error(`[BOT] Failed to start:`, err));
        bot.on('exit', (code, signal) => console.log(`[BOT] Exited — code: ${code}, signal: ${signal}`));
        app.on('before-quit', () => bot.kill());
    }, 2000);
    // ───────────────────────────────────────────────────────
